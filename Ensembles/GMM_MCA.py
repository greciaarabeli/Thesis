...
from Ensembles import quality_diversity_scores
import prince
...
def emProcess(list_ensembles,nEnsCluster, iterations, data):
    #let just partitions with final score higher than the mean using quality and diversity
    #do MCA for let just one componenet
    pca_matrix=do_mca(list_ensembles)
    N=len(pca_matrix) 
    H = 1
    M=nEnsCluster

    df = pd.DataFrame(pca_matrix, columns= np.arange(H))
    df.index.name = 'objs'
...
def do_gmm_mca(list_ensembles, nEnsCluster, iterations, verbose, N_clusters_max, hdf5_file_name, data):
    mixtureObj = emProcess(list_ensembles, nEnsCluster, iterations, data)
    return mixtureObj