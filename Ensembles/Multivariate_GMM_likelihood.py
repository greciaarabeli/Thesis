...
def EStep(X, w, mu, cov, N, M, H):
    ...
    for i in range(N):
        ...
        for j in range(M):
            r_ij_Sumj[j] = w[j] * stats.multivariate_normal.pdf(X[i,:], mu[j,:], cov[:,:,j])
        for j in range(M):
...
def emProcess(list_ensembles, nEnsCluster, iterations):
    # N: number of users, H number of clusterings for the ensemble.
    ...
    sdDiff = 4
    SDClass = np.random.rand(1,nEnsCluster)+sdDiff
    ...
    initw=np.ones(nEnsCluster) / nEnsCluster
    ...
    for init in range(Initializations):
        ...
        r_n = EStep(X, initw, initMu, initCov, N,nEnsCluster, H)
        w_n,mu_n,cov_n = MStep(r_n, X, initMu, initCov, N, nEnsCluster, H)
        ...
        for i in range(EMiteration):
            ...
            # E step
            r_n = EStep(X, w_n, mu_n, cov_n, N,nEnsCluster,H)

            # M step
            w_n,mu_n,sigma_n = MStep(r_n, X, mu_n, cov_n, N,nEnsCluster,H)
            ...
            for i in range(N):
                ...
                LH = np.zeros(nEnsCluster)

                for jClass in range(nEnsCluster):
                    LH[jClass] = w_n[jClass] * stats.multivariate_normal.pdf(X[i,:], mu_n[jClass,:], cov_n[:,:,jClass])
    ...
    final_prob=EStep(X, w_p, mu_p, sigma_p, N, nEnsCluster, H)
    
    final_part=np.empty([N])
    for i in range(N):
        final_part[i]=np.argmax(final_prob[i])
        
    return final_part, logLH
...
'''Input    
    ...
    Outputmax 
    a list with the labels for the final ensemble'''
